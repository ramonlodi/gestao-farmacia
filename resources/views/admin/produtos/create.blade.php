        ...
        <div class="card border-0 shadow-sm rounded-4 p-2">
            <div class="card-body p-4">
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div>
                        <h5 class="mb-0 fw-700">Novo Produto</h5>
                        <p class="text-muted small mb-0">Preencha os dados do produto</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('admin.produtos.store') }}" enctype="multipart/form-data" id="form-create">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="editor-create" style="height:150px;background:white;border-radius:0 0 8px 8px;"></div>
                        <input type="hidden" name="descricao" id="descricao-create">
                    </div>

                    <div class="row">
                        <div class="col mb-3">
                            <label class="form-label fw-500 small">Estoque</label>
                            <input type="number" name="estoque" class="form-control bg-light border-0" value="{{ old('estoque', 0) }}" min="0" required>
                        </div>
                        <div class="col mb-3">
                            <label class="form-label fw-500 small">Valor (R$)</label>
                            <input type="number" name="valor" class="form-control bg-light border-0" value="{{ old('valor') }}" step="0.01" min="0" required>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="fornecedores[]" class="form-select bg-light border-0" multiple size="4" required>
                            @foreach($fornecedores as $f)
                                <option value="{{ $f->id }}" {{ in_array($f->id, old('fornecedores', [])) ? 'selected' : '' }}>
                                    {{ $f->razao_social }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-500 small">Fotos (máx. 5) — serão redimensionadas para 800×800</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('form-create').addEventListener('submit', function () {
        document.getElementById('descricao-create').value = quill.root.innerHTML;
    });
