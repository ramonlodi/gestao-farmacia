            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center gap-3 mb-4">
                    <div style="width:44px;height:44px;background:#f0fdf4;border-radius:12px;display:flex;align-items:center;justify-content:center;">
                        <i class="bi bi-pencil" style="color:#15803D;"></i>
                    </div>
                    <h5 class="mb-0 fw-700">Editar Categoria</h5>
                </div>
                ...
                <form method="POST" action="{{ route('admin.categorias.update', $categoria) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nome" class="form-control bg-light border-0" value="{{ old('nome', $categoria->nome) }}" required>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="categoria_pai_id" class="form-select bg-light border-0">
                            ...
                            @foreach($categorias as $cat)
                                <option value="{{ $cat->id }}" {{ old('categoria_pai_id', $categoria->categoria_pai_id) == $cat->id ? 'selected' : '' }}>{{ $cat->nome }}</option>
                            @endforeach
                        </select>
                    </div>
