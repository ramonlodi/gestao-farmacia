        ...
        <div class="card border-0 shadow-sm rounded-4 p-2">
            <div class="card-body p-4">
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div>
                        <h5 class="mb-0 fw-700">Editar Endereço</h5>
                        <p class="text-muted small mb-0">Atualize os dados do endereço</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('cliente.enderecos.update', $endereco) }}">
                    @csrf @method('PUT')
                    <div class="mb-3">
                        <label class="form-label fw-500 small">Descrição</label>
                        <input type="text" name="descricao" class="form-control bg-light border-0" value="{{ old('descricao', $endereco->descricao) }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-500 small">CEP</label>
                        <input type="text" name="cep" class="form-control bg-light border-0"
                            value="{{ old('cep', $endereco->cep) }}" placeholder="00000-000">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-500 small">Logradouro</label>
                        <input type="text" name="logradouro" class="form-control bg-light border-0" value="{{ old('logradouro', $endereco->logradouro) }}" required>
                    </div>
                    <div class="row">
                        <div class="col mb-3">
                            <label class="form-label fw-500 small">Número</label>
                            <input type="text" name="numero" class="form-control bg-light border-0" value="{{ old('numero', $endereco->numero) }}" required>
                        </div>
                        <div class="col mb-3">
                            <label class="form-label fw-500 small">Bairro</label>
                            <input type="text" name="bairro" class="form-control bg-light border-0" value="{{ old('bairro', $endereco->bairro) }}" required>
                        </div>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="cidade_id" class="form-select bg-light border-0" required>
                            ...
                            @foreach($cidades as $cidade)
                                <option value="{{ $cidade->id }}" {{ old('cidade_id', $endereco->cidade_id) == $cidade->id ? 'selected' : '' }}>
